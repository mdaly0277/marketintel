...
interface TickerData {
  ...
  tier_perf: Record<string, { n: number; avg: number; win: number }>;
  history: HistoryPoint[];
}

const TIERS = [
  { key: "Positive", range: "75-100", color: "#34d399", bg: "rgba(52,211,153,0.08)", badge: "border-emerald-700/50 bg-emerald-950/40 text-emerald-300" },
  { key: "Neutral", range: "40-74", color: "#71717a", bg: "rgba(113,113,122,0.05)", badge: "border-zinc-700/50 bg-zinc-800/40 text-zinc-400" },
  { key: "Negative", range: "0-39", color: "#f87171", bg: "rgba(248,113,113,0.06)", badge: "border-red-800/50 bg-red-950/40 text-red-400" },
] as const;
...
export default function TickerPage() {
  ...
  const chartData = useMemo(() => {
    ...
    if (!tf || tf.weeks === Infinity) return data.history;
    const cutoff = data.history.length - tf.weeks;
    return data.history.slice(Math.max(0, cutoff));
  }, [data, timeframe]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black text-zinc-100 pt-14">
        <main className="mx-auto max-w-[960px] px-5 pt-12">
          <div className="animate-pulse space-y-6">
            <div className="h-6 w-24 bg-zinc-800/60 rounded" />
            <div className="h-10 w-28 bg-zinc-800 rounded-lg" />
            <div className="h-[300px] bg-zinc-900/30 rounded-2xl border border-zinc-800/30" />
          </div>
        </main>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-black text-zinc-100 pt-14">
        <main className="mx-auto max-w-[960px] px-5 pt-20 text-center">
          <h1 className="text-xl font-bold text-zinc-200 mb-2">{symbol ? `${symbol} not found` : "No ticker specified"}</h1>
          <p className="text-sm text-zinc-500 mb-8">This stock may not be in our scoring universe.</p>
          <a href="/screener" className="inline-block rounded-xl px-6 py-3 text-sm font-bold text-white"
            style={{ background: "linear-gradient(135deg, #3b82f6, #6366f1)" }}>Back to Screener</a>
        </main>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 pt-14">
      <div aria-hidden className="pointer-events-none fixed inset-0 overflow-hidden">
        <div className="absolute -top-[300px] -left-[150px] h-[700px] w-[700px] rounded-full opacity-[0.06]"
      ...
      <main className="relative mx-auto max-w-[960px] px-4 sm:px-5 pb-16 pt-6 sm:pt-10">
        <FadeIn>
          <a href="/screener" className="inline-flex items-center gap-1.5 text-[11px] text-zinc-600 hover:text-zinc-400 transition-colors mb-6">\u2190 Screener</a>
        </FadeIn>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between mb-6">
            <div>
              <div className="flex items-center gap-3 mb-1">
                <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-zinc-50">{data.ticker}</h1>
                <span className={`inline-block rounded-lg border px-2.5 py-1 text-[10px] font-bold tracking-wider uppercase ${tierInfo.badge}`}>{tierInfo.key}</span>
              </div>
              {data.name && <p className="text-sm text-zinc-500">{data.name}{data.sector && <span className="text-zinc-700 ml-1.5">\u00b7 {data.sector}</span>}</p>}
            </div>
            <div className="flex items-end gap-5">
              <div>
                <div className="text-[10px] uppercase tracking-wider text-zinc-600 mb-0.5">Score</div>
                <div className="text-4xl font-extrabold tabular-nums" style={{ color: tierInfo.color }}>{data.current_score.toFixed(1)}</div>
              </div>
              {chartData.length > 0 && chartData[chartData.length - 1].p != null && (
                <div>
                  ...
                  <div className="text-2xl font-bold tabular-nums text-zinc-200">{fmtPrice(chartData[chartData.length - 1].p!)}</div>
                </div>
              )}
            </div>
          </div>
        ...
        <FadeIn delay={100}>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-5">
            {([["1M Return", data.returns["1m"]], ["3M Return", data.returns["3m"]], ["6M Return", data.returns["6m"]], ["12M Return", data.returns["12m"]]] as [string, number | null][]).map(([label, raw]) => (
              <div key={label} className="rounded-xl border border-zinc-800/40 bg-zinc-950/60 px-3.5 py-3">
                <div className="text-[10px] text-zinc-600 uppercase tracking-wide">{label}</div>
                <div className={`text-lg sm:text-xl font-bold tabular-nums mt-0.5 ${retColor(raw)}`}>{fmtRet(raw)}</div>
              </div>
            ))}
          </div>
        </FadeIn>
            ...
            <div className="flex gap-1">
              {TIMEFRAMES.map((tf) => (
                <button key={tf.label} onClick={() => setTimeframe(tf.label)}
                  className={`rounded-lg px-3 py-1.5 text-[11px] font-bold tracking-wide transition-all ${timeframe === tf.label ? "bg-zinc-800 text-zinc-100" : "text-zinc-600 hover:text-zinc-400"}`}>
                  {tf.label}
                </button>
              ))}
            </div>
        ...
        {Object.keys(data.tier_perf).length > 0 && (
          <FadeIn delay={300}>
            <div className="mt-6">
              <h2 className="text-sm font-bold text-zinc-300 mb-1">How {data.ticker} performed in each tier</h2>
              <p className="text-[11px] text-zinc-600 mb-4">Average 3-month forward return when {data.ticker} was in each scoring tier.</p>
              <div className="rounded-xl border border-zinc-800/40 bg-zinc-950/50 overflow-hidden">
                <table className="w-full text-[12px] sm:text-[13px]">
                  <thead>
                    <tr className="border-b border-zinc-800/50">
                      <th className="text-left px-4 py-3 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Tier</th>
                      <th className="text-right px-4 py-3 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Times</th>
                      <th className="text-right px-4 py-3 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Avg 3M</th>
                      <th className="text-right px-4 py-3 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Win Rate</th>
                    </tr>
                  </thead>
                  <tbody>
                    {TIERS.map((tier) => {
                      ...
                      if (!perf) return null;
                      return (
                        <tr key={tier.key} className="border-b border-zinc-800/20 last:border-0 hover:bg-zinc-900/30">
                          <td className="px-4 py-3"><div className="flex items-center gap-2">
                            <span className="h-2 w-2 rounded-full" style={{ backgroundColor: tier.color, opacity: 0.8 }} />
                          ...
                          <td className={`px-4 py-3 text-right font-bold tabular-nums ${perf.avg >= 0 ? "text-emerald-400" : "text-red-400"}`}>{perf.avg >= 0 ? "+" : ""}{perf.avg.toFixed(1)}%</td>
                          <td className="px-4 py-3 text-right tabular-nums text-zinc-300">{perf.win.toFixed(0)}%</td>
                        </tr>
                      );
                    })}
                  </tbody>
        ...
        <FadeIn delay={350}>
          <div className="mt-8 flex flex-wrap gap-2.5">
            <a href="/screener" className="rounded-xl px-5 py-2.5 text-sm font-bold text-white"
              style={{ background: "linear-gradient(135deg, #3b82f6, #6366f1)" }}>Full Screener</a>
            <a href="/dashboard" className="rounded-xl border border-zinc-800 bg-zinc-950 px-5 py-2.5 text-sm font-medium text-zinc-400 hover:border-zinc-600 hover:text-zinc-200 transition-all">Dashboard</a>
            <a href="/portfolio" className="rounded-xl border border-zinc-800 bg-zinc-950 px-5 py-2.5 text-sm font-medium text-zinc-400 hover:border-zinc-600 hover:text-zinc-200 transition-all">Model Portfolio</a>
          </div>
        </FadeIn>

        <FadeIn delay={400}>
          <p className="mt-8 text-[11px] text-zinc-700">Not financial advice. Past performance does not guarantee future results.</p>
        </FadeIn>
      </main>
    </div>
  );
